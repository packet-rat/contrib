//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2014.04.17 at 04:06:30 PM EDT 
//


package org.mitre.cybox.common_2;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for RegionalRegistryTypeEnum.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="RegionalRegistryTypeEnum">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="AfriNIC"/>
 *     &lt;enumeration value="ARIN"/>
 *     &lt;enumeration value="APNIC"/>
 *     &lt;enumeration value="LACNIC"/>
 *     &lt;enumeration value="RIPE NCC"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "RegionalRegistryTypeEnum")
@XmlEnum
public enum RegionalRegistryTypeEnum {


    /**
     * AfriNIC stands for African Network Information Centre, and is the RIR for Africa.
     * 
     */
    @XmlEnumValue("AfriNIC")
    AFRI_NIC("AfriNIC"),

    /**
     * ARIN stands for American Registry for Internet Numbers, and is the RIR for the United States, Canada, several parts of the Caribbean Region, and Antarctica.
     * 
     */
    ARIN("ARIN"),

    /**
     * APNIC stands for Asia-Pacific Network Information Centre, and is the RIR for Asia, Australia, New Zealand, and neighboring countries.
     * 
     */
    APNIC("APNIC"),

    /**
     * LACNIC stands for Latin American and Caribbean Network Information Centre, and is the RIR for Latin America and parts of the Caribbean region.
     * 
     */
    LACNIC("LACNIC"),

    /**
     * RIPE NCC stands for Réseaux IP Européens Network Coordination Centre, and is the RIR for Europe, Russia, the Middle East, and Central Asia.
     * 
     */
    @XmlEnumValue("RIPE NCC")
    RIPE_NCC("RIPE NCC");
    private final String value;

    RegionalRegistryTypeEnum(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static RegionalRegistryTypeEnum fromValue(String v) {
        for (RegionalRegistryTypeEnum c: RegionalRegistryTypeEnum.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
